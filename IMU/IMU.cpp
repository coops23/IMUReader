...
IMU::IMU(void)
{
    _GyroMeasError = PI * (40.0f / 180.0f);
    _beta = sqrt(3.0f / 4.0f) * _GyroMeasError;
    _GyroMeasDrift = PI * (2.0f / 180.0f);
    _zeta = sqrt(3.0f / 4.0f) * _GyroMeasDrift;
    _q[0] = 1.0f;
    _q[1] = 0.0f;
    _q[2] = 0.0f;
    _q[3] = 0.0f;
...
int IMU::Initialize(float timeConstant)
{
    ...
    if (c == MPU6050_ADDRESS) // WHO_AM_I should always be 0x68
    {
        ...
        {
            calibrateMPU6050(_gyroBias, _accelBias); // Calibrate gyro and accelerometers, load biases in bias registers  
            initMPU6050(); Serial.println("MPU6050 initialized for active data mode...."); // Initialize device for active mode read of accelerometer, gyroscope, and temperature
        }
...
int IMU::Initialize(float timeConstant, uint8_t address)
{
    ...
    if (c == address)
    {
        ...
        {
            calibrateMPU6050(_gyroBias, _accelBias); // Calibrate gyro and accelerometers, load biases in bias registers  
            initMPU6050();  // Initialize device for active mode read of accelerometer, gyroscope, and temperature
        }
...
/*
This method needs to be ran as much as possible in the main program loop. It is essential for the filter algorithm.
The raw data is also pulled in this loop if data is available on the bus. The return type will indicate the presence
of new data which is useful for debugging throughput problems.
*/
int IMU::Update()
{
    int newData = 0;
    ...
    if(readByte(_address, INT_STATUS) & 0x01) {  // check if data ready interrupt
        ...
        float rollGyro = 0, rollAccel = 0;
       
        readAccelData(_accelCount);  // Read the x/y/z adc values
        getAres();
        ...
        _gx = (float)_gyroCount[0]*_gRes;  // get actual gyro value, this depends on scale being set
        _gy = (float)_gyroCount[1]*_gRes;  
        _gz = (float)_gyroCount[2]*_gRes;   
        ...
        rollGyro = ((_roll * 1000) + (_gy * _deltat));
        _yaw = ((_yaw * 1000) + (_gz * _deltat));
        pitchGyro /= 1000; 
        rollGyro /= 1000;
        _yaw /= 1000;
        ...
        _pitch = alpha * (pitchGyro) + (1 - alpha) * pitchAccel;
        _roll = alpha * (rollGyro) + (1 - alpha) * rollAccel;
        
        //Convert to quaternion
        float t0 = cos(_yaw * 0.5f);
        float t1 = sin(_yaw * 0.5f);
        float t2 = cos(_pitch * 0.5f);
        float t3 = sin(_pitch * 0.5f);
        float t4 = cos(_roll * 0.5f);
        float t5 = sin(_roll * 0.5f);

        _q[0] = t0 * t2 * t4 + t1 * t3 * t5;
...
float IMU::GetQuaternion0()
{
    return _q[0];
}

float IMU::GetQuaternion1()
{
    return _q[1];
}

float IMU::GetQuaternion2()
{
    return _q[2]; 
}

float IMU::GetQuaternion3()
{
    return _q[3];  
}
...
void IMU::MadgwickSensorFusionAlgorithm(float ax, float ay, float az, float gx, float gy, float gz)
{
...
{  
    ...
    writeByte(_address, SMPLRT_DIV, 0x04);  // Use a 200 Hz rate; the same rate set in CONFIG above

    // Set gyroscope full scale range
    // Range selects FS_SEL and AFS_SEL are 0 - 3, so 2-bit values are left-shifted into positions 4:3
    uint8_t c =  readByte(_address, GYRO_CONFIG);
    writeByte(_address, GYRO_CONFIG, c & ~0xE0); // Clear self-test bits [7:5] 
    writeByte(_address, GYRO_CONFIG, c & ~0x18); // Clear AFS bits [4:3]
    writeByte(_address, GYRO_CONFIG, c | _Gscale << 3); // Set full scale range for the gyro
    ...
    writeByte(_address, ACCEL_CONFIG, c | _Ascale << 3); // Set full scale range for the accelerometer 
...
void IMU::calibrateMPU6050(float * dest1, float * dest2)
{  
    ...
    writeByte(_address, PWR_MGMT_1, 0x80); // Write a one to bit 7 reset bit; toggle reset device
    delay(100);  
    ...
    writeByte(_address, FIFO_EN, 0x78);     // Enable gyro and accelerometer sensors for FIFO  (max size 1024 bytes in MPU-6050)
    ...
    writeByte(_address, XG_OFFS_USRH, data[0]);// might not be supported in MPU6050
    writeByte(_address, XG_OFFS_USRL, data[1]);
    writeByte(_address, YG_OFFS_USRH, data[2]);
    writeByte(_address, YG_OFFS_USRL, data[3]);
    writeByte(_address, ZG_OFFS_USRH, data[4]);
    writeByte(_address, ZG_OFFS_USRL, data[5]);
    ...
    writeByte(_address, XA_OFFSET_H, data[0]); // might not be supported in MPU6050
...
// Accelerometer and gyroscope self test; check calibration with factory settings
void IMU::MPU6050SelfTest(float * destination) // Should return percent deviation from factory trim values, +/- 14 or less deviation is a pass
{
    uint8_t rawData[4];
    uint8_t selfTest[6];
    float factoryTrim[6];
    ...
    writeByte(_address, ACCEL_CONFIG, 0xF0); // Enable self test on all three axes and set accelerometer range to +/- 8 g
    writeByte(_address, GYRO_CONFIG,  0xE0); // Enable self test on all three axes and set gyro range to +/- 250 degrees/s
    delay(250);  // Delay a while to let the device execute the self-test
    rawData[0] = readByte(_address, SELF_TEST_X); // X-axis self-test results
    rawData[1] = readByte(_address, SELF_TEST_Y); // Y-axis self-test results
    rawData[2] = readByte(_address, SELF_TEST_Z); // Z-axis self-test results
    rawData[3] = readByte(_address, SELF_TEST_A); // Mixed-axis self-test results
...
uint8_t IMU::readByte(uint8_t address, uint8_t subAddress)
{
...
/*
TODO: If needed. Allows for power to be saved when device is idle.

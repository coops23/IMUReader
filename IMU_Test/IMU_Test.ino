...
#define G_TO_MS3_CONVERSION (9.80665f)
...
// Pin definitions
int intPin = 9;  // These can be changed, 2 and 3 are the Arduinos ext int pins
...
uint32_t before = 0, Period = 0;
float q[4] = {1.0f, 0.0f, 0.0f, 0.0f};            // vector to hold quaternion

float velocityPrevious[3] = {0, 0 ,0};
float positionPrevious[3] = {0, 0 ,0};
...
{  
  float positionInfo[3];
    ...
    delt_t = millis() - count;
    //if (delt_t > 16) { // update LCD once per half-second independent of read rate 
    ...
    SendVisualizationData(yaw, pitch, roll, ax, ay, az, q[0], q[1], q[2], q[3], delt_t);   

        
        
        //SendVisualizationData(yaw, pitch, roll, ax, ay, az, q[0], q[1], q[2], q[3], delt_t);
        //Serial.print("average rate = "); Serial.print((Period/1000)); Serial.println(" Hz");
...
// Configure MPU6050 gyro and accelerometer for bias calculation
  ...
  uint16_t  accelsensitivity = 16384;  // = 16384 LSB/g
   //uint16_t  accelsensitivity = 8192;  // = 16384 LSB/g
...
//Used to send data to processing sketch for IMU visualization
//Send orientation in degrees and position in mm
void SendVisualizationData(float yaw, float pitch, float roll, float ax, float ay, float az, float qs, float qx, float qy, float qz, float delta_t)
{
    //Serial.print("Info: ");
    Serial.print(yaw);
    Serial.print(",");
